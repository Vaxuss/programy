...
namespace Hobby
{
    internal class HobbyEventManager
    {
        ...
        public List<Member> UserList = new List<Member>();
        public int run()
        {
            LoginManager loginManager = new LoginManager();
            Member admin = new Member("Artur", "Derybowski", "admin", "admin", 1);
            UserList.Add(admin);
            Console.WriteLine("Login: ");
            string login = Console.ReadLine();
            Console.WriteLine("Hasło: ");
            string pass = Console.ReadLine();
            int m = loginManager.Login(login, pass);
            if (UserList[m].accountType == 0)
            {
                UserHobbyManager user = new UserHobbyManager();
                user.RunAccount();
            }
            else
            {
        ...
        public void AddUser(LoginManager login)
        {
            Console.Clear();
